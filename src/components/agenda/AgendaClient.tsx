...
export function AgendaClient({ schedule, freeCars = [], freeTracks = [] }: Props) {
  ...
  const [selectedSeries, setSelectedSeries] = useState<{ series: IRacingSeries, week: SeriesWeek } | null>(null);
  ...
  const isCarOwned = (series: IRacingSeries) => {
    ...
    if (!showOnlyOwned) {
      ...
      if (legacyFreeCars.some(c => series.seriesName.includes(c))) return true;
    }
    return false;
  };
    ...
    <div className="animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-10">
      ...
      {selectedSeries && (
        <SeriesInfoModal 
          ...
          currentWeek={selectedSeries.week}
        />
      )}
    </div>
  );
